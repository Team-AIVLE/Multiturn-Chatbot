...
'''
def beam_search(model, tokenizer, qurey,device):
  input_ids = tokenizer(qurey,return_tensors='pt')['input_ids'][:,:128-1].to(device=device)
  #print(input_ids.shape)
  search = model.generate(input_ids, num_beams=3,
                                  no_repeat_ngram_size=2,
                                  max_length=256,
                                  num_return_sequences=3,
                                  early_stopping=True)
  return list(map(lambda x: tokenizer.decode(x[min(128,x.shape[0]):]), search))
...
def eval_ar(args, model, device, test_data):
    ...
    with torch.no_grad():
        for d in test_data.iterrows():
            ...
            # inference
            sys_response = beam_search(model,tokenizer,u_tkn+query+s_tkn,device)
            #sys_response= sys_response.strip()
            reply = list(map(lambda x: proc_reply(x.strip()),sys_response))
